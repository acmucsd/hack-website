...
{/* ![image that I cant load](pages\hack\consolelog.png) */}
...
### let
Let is block scoped, can be updated but no redeclared
```javascript copy showLineNumbers
let greeting = "say Hi";
...
if (true) {
    let freeting = "say Hello instead";
...
## Loops
Loops allow you to execute a block of code repeatedly. Common loop types are `for`, `while`, and `do-while`.
