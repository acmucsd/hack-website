...
## What is Express?
Express is a web application framework for Node.js that enables developers to start a local server and develop API routes. Express does
a good job of abstracting away the complexity of serving a web application.
...
Generally, APIs provide API Routes, which are specific routes on a server that HTTP requests can be sent to.
